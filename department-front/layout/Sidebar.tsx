...
import { LayoutDashboard, Database, CreditCard, FileText, Activity, UserCog, Filter, Settings, Shield, ChevronRight, LogOut } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ activeItem, onNavigate, onLogout }) => {
  return (
    <aside className="w-72 bg-tmt-black border-r border-tmt-emerald/20 h-full flex flex-col hidden md:flex z-30">
      <div className="h-20 flex items-center px-8 border-b border-white/10">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-white font-bold text-base tracking-tight">TMT Department</h1>
            <p className="text-[10px] text-tmt-emerald font-bold tracking-widest uppercase">Management Suite</p>
          </div>
        </div>
      </div>
      ...
      <nav className="flex-1 overflow-y-auto py-6 px-4 space-y-8">
        <div>
          <p className="px-4 text-[10px] font-bold text-tmt-emerald/90 uppercase tracking-[0.2em] mb-4">Main Navigation</p>
        ...
        <div>
          <p className="px-4 text-[10px] font-bold text-tmt-emerald/90 uppercase tracking-[0.2em] mb-4">Administration</p>
          <ul className="space-y-1.5">
      ...
      <div className="p-6 mt-auto space-y-3">
        <button 
          onClick={onLogout}
          className="w-full flex items-center px-4 py-3 text-sm font-medium text-rose-500 hover:bg-rose-500/10 rounded-xl transition-all group border border-transparent hover:border-rose-500/20"
        >
        ...
        <div className="bg-white/5 border border-white/10 p-4 rounded-2xl flex items-center gap-4 shadow-2xl">
            ...
            <div>
                <p className="text-xs font-bold text-white">Security Verified</p>
                <div className="flex items-center gap-1.5 mt-0.5">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>
                  <p className="text-[10px] text-tmt-emerald font-bold">System Status: Optimal</p>
                </div>
            </div>
        </div>
      </div>
